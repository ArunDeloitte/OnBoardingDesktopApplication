    ...
    public partial class AdminReport : Form
    {
        ...
        private void btnDownload_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 1; i < dgvAdminReport.Columns.Count + 1; i++)
                {
                    worksheet.Cells[1, i] = dgvAdminReport.Columns[i - 1].HeaderText;
                }



                // storing Each row and column value to excel sheet
                for (int i = 0; i < dgvAdminReport.Rows.Count - 1; i++)
                {
                    for (int j = 0; j < dgvAdminReport.Columns.Count; j++)
                    {
                        worksheet.Cells[i + 2, j + 1] = dgvAdminReport.Rows[i].Cells[j].Value.ToString();
                    }
                }
